...
class Approvaltotal extends Model
{
    ...
    public static function overview($student_year,$student_class,$experiment_name)
    {
        ...
        try{
                ...
                $cegs = ['student_year'=>$student_year,'student_class'=>$student_class,'experiment_name'=>$experiment_name];
                $row = Approvaltotal::select('id','student_num','student_name','experiment_name','student_class','postate','state','grade')
                    ->where($cegs)
                    ->get();
