...
if (!function_exists('effective')) {
      /**
     * 接收字符串
     * 判断数字有效位数（只能是数字）
     */
    function effective($num)
    {
        $res =$num;
          $len = strlen($res);
          $flag = true;
...
if (!function_exists('geteffective')) {

    /**
     * 把得到的数字转换成 有效位数
     */
    function geteffective($res,$num)
    {
        $res=strval($res);  //转换 字符串
        $len = strlen($res);
        $flag=false;
        for($i = 0;$i < $len;$i++) {
            if ($res[$i] == '.') continue; //小数点 下一位
                if (($res[$i] != '0')) //开始第一位
                {
                  $flag=true;
                    $num--;
